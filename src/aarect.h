...
public:
	...
	virtual bool bounding_box(double time0, double time1, aabb& output_box)
		...
		// The bounding box must have non-zero width in each dimension, so pad the Z
			...
			output_box = aabb(point3(x0, y0, k - 0.0001), point3(x1, y1,
				k + 0.0001));
		return true;
	}
public:
...
public:
	...
	virtual bool bounding_box(double time0, double time1, aabb& output_box)
		const override {
		// The bounding box must have non-zero width in each dimension, sopad the Y
			// dimension a small amount.
			output_box = aabb(point3(x0, k - 0.0001, z0), point3(x1, k + 0.0001,
				z1));
		return true;
	}
public:
...
public:
	...
	virtual bool bounding_box(double time0, double time1, aabb& output_box)
		...
		// The bounding box must have non-zero width in each dimension, so pad the X
			// dimension a small amount.
			output_box = aabb(point3(k - 0.0001, y0, z0), point3(k + 0.0001, y1,
				z1));
		return true;
	}
public:
